	...
	*/
	private function _verify_CE_CL_EP_Q_QC_is_Match(){
		...
		elseif(isset($this->infoForAdditionalValidate['addEditMainEntity'])){
			...
			if (select COUNT(*) from hds_devClassEnrollists cel inner join hds_devClassExamPapers cep on cel.classId=cep.classId inner join hds_devQuizAnswerObjectives qao on cel.id=qao.classEnrollListId and cep.id=qao.classExamPaperId where qao.id=@thisFieldVal) = 0
			begin 
				set @errorMessage = 'Class Enrollment and Exam paper Mismatch.';  
				goto Skipper; 
			end
			
			if (select COUNT(*) cnt from hds_devQuizzes q inner join hds_devClassExamPapers cep on q.id=cep.quizId inner join hds_devQuizChoices qc on qc.quizId=q.id inner join hds_devQuizAnswerObjectives qao on qao.classExamPaperId=cep.id and qao.quizChoiceId=qc.id where qao.id=@thisFieldVal) = 0
			begin 
				set @errorMessage = 'Exam paper and Choice Selected is Mismatch. ';  
				goto Skipper; 
			end
